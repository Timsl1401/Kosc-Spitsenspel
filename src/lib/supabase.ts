...
export const getActiveSeason = async (): Promise<Season | null> => {
  try {
    ...
    // Temporary fallback: return a default season if table doesn't exist
    if (error.code === 'PGRST205' || error.message?.includes('Could not find the table')) {
      console.warn('Seasons table not found. Please apply the database schema from improved_database_schema.sql')
      return {
        id: 'default-season',
        name: 'Default Season',
...
export const getUserTeam = async (userId: string, seasonId: string): Promise<UserTeam[]> => {
  try {
    // Temporary: remove season join until seasons table is created
    const { data, error } = await supabase
      .from('user_teams')
      .select(`
        *,
    ...
    // If seasons table doesn't exist, try without season join
    if (error.code === 'PGRST205' || error.message?.includes('Could not find the table')) {
      console.warn('Seasons table not found. Please apply the database schema from improved_database_schema.sql')
      try {
