    ...
    <div class="relative">
        <div class="flex justify-between items-center mb-1">
            <div class="flex-grow flex items-center">
                ...
                <div class="w-96 flex rounded-lg shadow-sm mt-3 mb-3">
                    <div class="relative flex-grow focus-within:z-10">
                        ...
                        <input wire:model.debounce.500ms="search"
                            class="form-input block bg-gray-50 focus:bg-white w-full rounded-md pl-10 transition ease-in-out duration-150 sm:text-sm sm:leading-5 py-2"
                            placeholder="{{__('Zoeken in')}} {{ $this->searchableColumns()->map->label->join(', ') }}" />
            ...
            <div class="rounded-lg @unless($this->hidePagination) rounded-b-none @endif">
                <div class="table align-middle min-w-full">
                    ...
                    <div class="table-row divide-x divide-gray-200">
                        ...
                        <div
                            class="w-32 py-4 flex justify-center overflow-hidden align-top px-6 border-b border-gray-200 bg-gray-50 text-left text-xs leading-4 font-medium text-gray-500 uppercase tracking-wider focus:outline-none">
                            <div
                        ...
                        <div
                            ...
                            <div>
                                <input type="checkbox" wire:click="toggleSelectAll"
                                    @if(count($selected)===$this->results->total()) checked @endif class="form-checkbox
                                mt-1 h-4 w-4 text-blue-600 transition duration-150 ease-in-out" />
                            </div>
                        </div>
                        @else
                    ...
                    <div
                        class="table-row p-1 divide-x divide-gray-100 {{ isset($result->checkbox_attribute) && in_array($result->checkbox_attribute, $selected) ? 'bg-orange-100' : ($loop->even ? 'bg-gray-100' : 'bg-gray-50') }}">
                    ...
                    <p class="p-3 text-lg text-teal-600">
                        {{ __("Er is niets om weer te geven op dit moment") }}
                    </p>
                    @endforelse
                </div>
            </div>
                ...
                <div class="p-2 sm:flex items-center justify-between">
                    ...
                    <div class="my-2 sm:my-0 flex items-center">
                        <select name="perPage"
                            ...
                            @endforeach
                            <option value="99999999">{{__('Alle')}}</option>
                        </select>
                    </div>
                    ...
                    <div class="flex justify-end text-gray-600">
                        {{__('Resultaten')}} {{ $this->results->firstItem() }} - {{ $this->results->lastItem() }}
                        {{__('van')}}
                        {{ $this->results->total() }}
                    </div>
                    @endif
